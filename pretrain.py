...
# Modified from CSWin (https://github.com/microsoft/CSWin-Transformer) and AlphaNet (https://github.com/facebookresearch/AlphaNet)
...
from timm.models import convert_splitbn_model, create_model, load_checkpoint, resume_checkpoint
...
parser = argparse.ArgumentParser(description="HRViT Training and Evaluating")
...
parser.add_argument("--ngpus-per-node", default=8, type=int, help="number of GPUs per node, distributed setting")
parser.add_argument("--dist-url", default="auto", type=str, help="init method, distributed setting")

# Dataset / Model parameters
parser.add_argument("--data", default="/mnt/blob/testset/ImageNet", metavar="DIR", help="path to dataset")
parser.add_argument(
    "--model",
    default="HRViT_b1_224",
    type=str,
    metavar="MODEL",
...
parser.add_argument(
    "--eval_checkpoint", default="", type=str, metavar="PATH", help="path to eval checkpoint (default: none)"
)
parser.add_argument(
    "--no-resume-opt",
    action="store_true",
...
parser.add_argument(
    "--in-channels", type=int, default=3, metavar="N", help="number of input channels (default: 3)"
)
parser.add_argument(
    "--num-classes", type=int, default=1000, metavar="N", help="number of label classes (default: 1000)"
)
parser.add_argument(
    "--gp",
    default=None,
...
parser.add_argument(
    "--weight-decay", type=float, default=0.05, help="weight decay (default: 0.005 for adamw)"
)
parser.add_argument(
    "--clip-grad",
    type=float,
    default=None,
    metavar="VALUE",
    help="Clip gradient value (default: None, no clipping)",
)
...
parser.add_argument(
    "--warmup-lr", type=float, default=1e-6, metavar="LR", help="warmup learning rate (default: 0.0001)"
)
parser.add_argument(
    "--min-lr",
    type=float,
...
parser.add_argument(
    "--epochs", type=int, default=300, metavar="N", help="number of epochs to train (default: 2)"
)
parser.add_argument(
...
parser.add_argument(
    "--warmup-epochs", type=int, default=20, metavar="N", help="epochs to warmup LR, if scheduler supports"
)
parser.add_argument(
    "--cooldown-epochs",
    type=int,
...
parser.add_argument(
    ...
    default=0.1,
    metavar="PCT",
    help="Drop path rate for the classification head (default: None)",
)
parser.add_argument(
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Turn on gradient checkpointing to save training memory cost",
)

has_apex = False  # force to not use apex amp
amp = None
...
def _spawn_and_print_error(process_idx: int, func: Callable[..., Any], *args: Any) -> None:
    try:
        func(process_idx, *args)
...
def main_worker(local_rank, ngpus_per_node, args):
    ...
    args.device = "cuda:0"
    args.rank = args.local_rank + args.machine_rank * ngpus_per_node  # global rank
    if args.distributed:
        args.num_gpu = 1
    ...
    # create model
    _logger.info("Creating model '{}'".format(args.model))
    model = create_model(
        args.model,
    ...
    if args.local_rank == 0:
        _logger.info("Model %s created, param count: %d" % (args.model, count_parameters(model)))
    ...
    loader_train, loader_eval, mixup_fn = data_loader.build_dataloader(args, data_config)
    ...
    if args.local_rank == 0:
        ...
        exp_name = "-".join(
            [datetime.now().strftime("%Y%m%d-%H%M%S"), args.model, str(data_config["input_size"][-1])]
        )
        ...
        with open(os.path.join(output_dir, "args.yaml"), "w") as f:
            f.write(args.args_text)

    _logger.info(args.args_text)
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            ...
            if comm.is_master_process() and saver is not None:
...
def main():
    ...
    assert torch.cuda.device_count() >= args.ngpus_per_node, f"Not enough GPUs per node, requires {args.ngpus_per_node}, but only has {torch.cuda.device_count()} GPUs per node"
    ...
    if args.distributed:
        ...
        mp.spawn(
            _spawn_and_print_error,
            nprocs=ngpus_per_node,
            args=(main_worker, ngpus_per_node, args),
            join=True,
        )
    else:
        args.world_size = 1
        ngpus_per_node = 1
        _spawn_and_print_error(0, main_worker, ngpus_per_node, args)
